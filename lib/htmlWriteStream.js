...
const changeToHtmlExt = require("./changeToHtmlExt");
...
const writeToHtmlstream = (filename, fileArray) => {
  ...
  const newFilename = changeToHtmlExt(filename)

  const fileLinks = fileArray.flatMap(file => {
    if (!file.startsWith(filename)) {
      const newFilename = changeToHtmlExt(file)
  ...
  <ul style="font-family: 'Roboto Mono', serif; color: rgba(0,0,0,0.9); display: flex; justify-content: center; align-items: center; align-content: center; gap: 0.5em; flex-wrap: wrap">
  ${fileLinks}
  </ul>
  </nav>`
